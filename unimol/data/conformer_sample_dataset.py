...
class ConformerSampleDataset(BaseWrapperDataset):
    ...
    def __cached_item__(self, index: int, epoch: int):
        ...
        res = {"atoms": atoms, "coordinates": coordinates.astype(np.float32)}
        if "smi_tokenized" in self.dataset[index]:
            res["smi_tokenized"] = self.dataset[index]["smi_tokenized"]
        for k in self.dataset[index].keys():
            if k not in res.keys():
                res[k] = self.dataset[index][k]
        if 'bond_targets' in self.dataset[index]:
            res['bond_targets'] = self.dataset[index]["bond_targets"][sample_idx]
            res['angle_targets'] = self.dataset[index]["angle_targets"][sample_idx]
            res['dihedral_targets'] = self.dataset[index]["dihedral_targets"][sample_idx]
