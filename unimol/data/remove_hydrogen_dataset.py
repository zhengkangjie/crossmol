...
class RemoveHydrogenDataset(BaseWrapperDataset):
    ...
    def __cached_item__(self, index: int, epoch: int):
        ...
        if self.remove_hydrogen:
            ...
            if 'edge_input' in dd:
                mol = Chem.MolFromSmiles(dd["smi"])
                mask_hydrogen_t = np.array([a.GetSymbol() != "H" for a in mol.GetAtoms()])
                mask_hydrogen_t = torch.from_numpy(mask_hydrogen_t)
                edge_input = torch.from_numpy(dd['edge_input'])
                edge_input = edge_input[mask_hydrogen_t, :, :, :]
                edge_input = edge_input[:, mask_hydrogen_t, :, :]
                spatial_pos = torch.from_numpy(dd['spatial_pos'])
                spatial_pos = spatial_pos[mask_hydrogen_t, :]
                spatial_pos = spatial_pos[:, mask_hydrogen_t]
                dd['edge_input'] = edge_input
                dd['spatial_pos'] = spatial_pos
