...
@register_model("smi2struct_zmat_head")
class Smi2StructZmatHeadModel(BaseUnicoreModel):
    @staticmethod
    def add_args(parser):
    ...
    def __init__(self, args, source_dictionary, target_dictionary):
        ...
        if args.encoder_dihedral_loss > 0:
    ...
    def forward(
        ...
        **kwargs
        ...
        # print('decoder_padding_mask size:', decoder_padding_mask.size())
        # print('tgt_tokens size:', tgt_tokens.size())
        # print('tgt_tokens_pos nonpadding len:', torch.max(torch.sum((tgt_tokens_pos!=0).long(), dim=-1)))
        # print('tgt_tokens_pos size:', tgt_tokens_pos.size())
        # print('atoms_features size:', atoms_features.size())
        # print('--------------------')
        

        copy_embedding = False
        if self.decoder_copy_embedding:
            assert tgt_tokens_pos is not None
            smi_length = (~encoder_padding_mask).long().sum(-1, keepdim=True) - 1
            atoms_num = (~decoder_padding_mask).long().sum(-1, keepdim=True) - 1
            tgt_tokens_pos_mod = torch.scatter(input=tgt_tokens_pos, dim=1, index=atoms_num, src=smi_length)# process [eos]
            x = torch.gather(input=encoder_output_embedding, dim=1, index=tgt_tokens_pos_mod.unsqueeze(-1).expand(-1, -1, encoder_output_embedding.size(-1)))
            copy_embedding = True
        else:
        ...
        if not decoder_padding_mask.any():
            ...
            # print('tgt_tokens size:', tgt_tokens.size())
            # print('x size:', x.size())
            # print('tgt_tokens_pos_mod size:', tgt_tokens_pos_mod.size())
            # print('tgt_tokens_pos size:', tgt_tokens_pos.size())
            # print('atoms_features size:', atoms_features.size())
            # print('--------------------')
...
@register_model_architecture("smi2struct_zmat_head", "smi2struct_zmat_head")
def base_architecture(args):
    ...
    args.no_cross_attn = getattr(args, "no_cross_attn", False)

    

@register_model_architecture("smi2struct_zmat_head", "smi2struct_zmat_head_base")
def unimol_base_architecture(args):
    base_architecture(args)

@register_model_architecture("smi2struct_zmat_head", "smi2struct_zmat_head_150M")
