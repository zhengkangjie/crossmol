...
@register_model("smi2struct_2d")
class Smi2Struct2DModel(BaseUnicoreModel):
    @staticmethod
    def add_args(parser):
    ...
    def __init__(self, args, source_dictionary, target_dictionary):
        ...
        num_edges = 512 * 3
        self.edge_encoder = nn.Embedding(num_edges + 1, args.decoder_attention_heads, padding_idx=0)
        self.edge_type = args.edge_type
        if self.edge_type == 'multi_hop':
            self.edge_dis_encoder = nn.Embedding(
                args.num_edge_dis * args.decoder_attention_heads * args.decoder_attention_heads, 1)
        
        num_spatial = 1024 * 3
        self.spatial_pos_encoder = nn.Embedding(num_spatial, args.decoder_attention_heads, padding_idx=0)
    ...
    def forward(
        ...
        **kwargs
        ...
        if self.decoder_copy_embedding and tgt_tokens_pos is not None:
            ...
            x = torch.gather(input=encoder_output_embedding, dim=1, index=tgt_tokens_pos.unsqueeze(-1).expand(-1, -1, encoder_output_embedding.size(-1)))
            copy_embedding = True
        else:
        ...
        print('edge_input size:', edge_input.size())
...
@register_model_architecture("smi2struct_2d", "smi2struct_2d")
...
@register_model_architecture("smi2struct_2d", "smi2struct_2d_base")
def unimol_base_architecture(args):
    base_architecture(args)

@register_model_architecture("smi2struct_2d", "smi2struct_2d_150M")
