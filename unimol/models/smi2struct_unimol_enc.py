...
from unicore.modules import TransformerEncoder
...
@register_model("smi2struct_unimol_enc")
class Smi2StructUniMolEncModel(BaseUnicoreModel):
    @staticmethod
    def add_args(parser):
    ...
    def __init__(self, args, source_dictionary, target_dictionary):
        ...
        self.encoder_embed_positions = SinusoidalPositionalEmbedding(
            embedding_dim = args.encoder_embed_dim,
            padding_idx = source_dictionary.pad(),
            init_size = args.max_source_positions,
        )

        if args.masked_smi_loss > 0:
            self.enc_lm_head = MaskLMHead(
                embed_dim=args.encoder_embed_dim,
                output_dim=len(source_dictionary),
                activation_fn=args.activation_fn,
                weight=None,
            )
        else:
            self.enc_lm_head = None

        self.encoder_embed_tokens = nn.Embedding(
            len(source_dictionary), args.encoder_embed_dim, self.encoder_padding_idx
        )
    ...
    def forward(
        ...
        **kwargs
        ...
        #     retuen_feature=True
        # )
        encoder_input_embedding = self.encoder_embed_tokens(smi_tokens) + self.encoder_embed_positions(smi_tokens)
        encoder_output_embedding = self.encoder(
            emb = encoder_input_embedding,
            padding_mask = encoder_padding_mask,
        )

        if self.args.masked_smi_loss <= 0 or self.enc_lm_head is None:
            encoder_logits = None
        else:
            encoder_logits = self.enc_lm_head(
                features=encoder_output_embedding, 
                masked_tokens=encoder_masked_tokens,
            )
        ...
        x = self.decoder_embed_tokens(tgt_tokens)
...
@register_model_architecture("smi2struct_unimol_enc", "smi2struct_unimol_enc")
...
@register_model_architecture("smi2struct_unimol_enc", "smi2struct_unimol_enc_base")
def unimol_base_architecture(args):
    base_architecture(args)

@register_model_architecture("smi2struct_unimol_enc", "smi2struct_unimol_enc_150M")
