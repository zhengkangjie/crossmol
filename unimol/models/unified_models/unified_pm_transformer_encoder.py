...
from .unified_pm_transformer_layer import UnifiedPMTransformerEncoderLayer
...
class UnifiedPMTransformerEncoder(FairseqEncoder):
    """
    ...
    def __init__(self, args, dictionary, embed_tokens, return_fc=False):
        ...
        self.emb_layer_norm_after = LayerNorm(embed_dim, export=False)

    def build_encoder_layer(self, args):
        layer = UnifiedPMTransformerEncoderLayer(
            args, return_fc=self.return_fc
        )
        return layer
    ...
    def forward_embedding(
        ...
        x = F.dropout(x, p=self.dropout_p, training=self.training, inplace=False)
        return x, embed
    ...
    def forward(
        ...
        not_valid_pair_mask: Optional[torch.Tensor] = None,
    ):
        """
        Args:
        ...
        return self.forward_scriptable(
            src_tokens, src_lengths, aa_mask, attn_bias, return_all_hiddens, token_embeddings, token_dropout, need_head_weights, return_contacts, not_valid_pair_mask=not_valid_pair_mask
        )
    ...
    def forward_scriptable(
        ...
        if not_valid_pair_mask is not None:
            # print("not_valid_pair_mask:", not_valid_pair_mask)
        ...
        x_norm = norm_loss(x.transpose(0, 1))
    ...
    def upgrade_state_dict_named(self, state_dict, name):
        ...
        if get_item(state_dict.get(version_key, torch.Tensor([1]))[0]) < 2:
